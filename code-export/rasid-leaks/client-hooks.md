# rasid-leaks - client-hooks

> Auto-extracted source code documentation

---

## `client/src/hooks/atlas/useComposition.ts`
...
## `client/src/hooks/atlas/useMobile.tsx`
...
## `client/src/hooks/atlas/usePersistFn.ts`
...
## `client/src/hooks/atlas/useThemeColors.ts`

```typescript
/**
 * useThemeColors — Returns theme-aware color palette for inline styles.
 * Components use this instead of hardcoding dark-only colors.
 */
import { useTheme } from "@/contexts/ThemeContext";
import { getThemeColors, getTooltipStyle, type ThemeColors } from "@/lib/atlas/themeColors";
...
## `client/src/hooks/atlas/useThemeLogo.ts`

```typescript
/**
 * useThemeLogo — Returns the appropriate logo variant based on current theme.
 * Dark mode: light/gold logos (for dark backgrounds)
 * Light mode: navy/dark logos (for light backgrounds)
 */
import { useTheme } from "@/contexts/ThemeContext";
...
export function useThemeLogo() {
  ...
  return {
    /** Calligraphy icon — sidebar/nav (gold for dark, navy for light) */
    logoIcon: isDark ? RASID_ASSETS.logoIcon : RASID_ASSETS.logoDarkVariant,
    /** Badge variant — small contexts */
    logoBadge: isDark ? RASID_ASSETS.logoBadge : RASID_ASSETS.logoDarkVariant,
    /** Full horizontal logo with text */
    logoHorizontal: isDark ? RASID_ASSETS.logoHorizontal : RASID_ASSETS.logoFullDark,
    /** Minimal calligraphy */
    logoMinimal: isDark ? RASID_ASSETS.logoMinimal : RASID_ASSETS.logoDarkVariant,
    /** Full login screen logo */
    logoLoginFull: isDark ? RASID_ASSETS.logoLoginFull : RASID_ASSETS.logoFullDark,
    /** Current theme state */
    isDark,
  };
}
 ...
 * useThemeColors — Returns theme-aware color palette for inline styles.
 * Components use this instead of hardcoding dark-only colors.
 * Synced with rasid-atlas design system.
 */
import { useTheme } from "@/contexts/ThemeContext";
import { getThemeColors, getTooltipStyle, type ThemeColors } from "@/lib/themeColors";
