      ...
      if (data.breaches && Array.isArray(data.breaches) && data.breaches.length > 0) {
        ...
        for (const breachName of breachList) {
          results.push({
            ...
            type: "breach",
            severity: "high",
            title: `حالة رصد بيانات من ${breachName}`,
            description: `تم العثور على البريد الإلكتروني ${emailToCheck} في حالة رصد بيانات ${breachName}`,
            ...
            for (const breach of analytics.ExposedBreaches.breaches_details) {
              ...
              if (!exists) {
                ...
                results.push({
                  ...
                  title: `حالة رصد ${breach.breach} — ${breach.industry || "غير محدد"}`,
                  description: breach.details || `حالة رصد بيانات من ${breach.breach}`,
                  details: {
...
async function scanGoogleDorks(target: ScanTarget): Promise<{ results: ScanResult[]; progress: ScanProgress }> {
  ...
  try {
    ...
    if (target.type === "domain") {
      ...
      dorkQueries.push(
        `"${target.value}" "حالة رصد" OR "leak" OR "breach"`,
        `"${target.value}" site:pastebin.com`,
  ...
  try {
    ...
    return {
      ...
      progress: {
        source,
        status: "completed",
        message: `تم فحص قاعدة بيانات حالات الرصد — ${results.length} نتيجة`,
        resultsCount: results.length,
        timestamp: new Date(),
      },
    };
  ...
  try {
    ...
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `أنت محلل أمن معلومات متخصص في حالات رصد البيانات الشخصية. قم بتحليل نتائج المسح التالية وقدم:
...
export async function executeScan(
  ...
  for (const target of targets) {
    ...
    if (enabledSources.includes("breachdirectory")) {
      addProgress({ source: "BreachDirectory", status: "scanning", message: `جارٍ فحص قاعدة بيانات حالات الرصد...`, resultsCount: 0, timestamp: new Date() });
