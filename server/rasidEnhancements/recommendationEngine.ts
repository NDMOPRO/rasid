...
export class RecommendationEngine {
      ...
      '/dashboard': [
        {
          ...
          description: 'قم بتحليل كيف تغيرت الحوادث خلال آخر 6 أشهر',
          confidence: 0.8,
          priority: 'medium',
          action: {
            type: 'generate_chart',
        ...
        },
        {
          ...
          description: 'أنشئ تقرير دوري شهري لملخص الحوادث',
          confidence: 0.7,
          priority: 'low',
          createdAt: new Date(),
        },
      ...
      '/leaks': [
        {
          ...
          description: 'قارن بين القطاع المالي والصحي من حيث عدد الحوادث',
          confidence: 0.75,
          priority: 'medium',
          action: {
      ...
      '/evidence': [
        {
          ...
          description: 'اكتشف العلاقات بين الحوادث المتصلة في سلسلة الأدلة',
          confidence: 0.85,
          priority: 'high',
          createdAt: new Date(),
        },
  ...
  private getQueryRecommendations(queries: string[]): Recommendation[] {
    ...
    if (sectorMatch) {
      ...
      recommendations.push({
        ...
        description: `احصل على تحليل شامل لحوادث القطاع ${sector} بما في ذلك الاتجاهات والأنماط`,
        confidence: 0.9,
        priority: 'high',
        action: {
          type: 'deep_analysis',
  ...
  private getSectorRecommendations(sectors: string[]): Recommendation[] {
    ...
    if (sectors.length >= 2) {
      ...
      recommendations.push({
        ...
        description: `قارن بين القطاعين من حيث عدد الحوادث، حجم البيانات، والخطورة`,
        confidence: 0.8,
        priority: 'medium',
        action: {
          type: 'compare_sectors',
  ...
  private getIncidentRecommendations(incidents: string[]): Recommendation[] {
    ...
    if (incidents.length >= 3) {
      recommendations.push({
        ...
        description: 'هل لاحظت أنماطاً مشتركة بين الحوادث التي شاهدتها؟ دعني أحللها لك',
        confidence: 0.7,
        priority: 'medium',
        createdAt: new Date(),
      });
  ...
  suggestNextAnalysis(currentAnalysis: {
    ...
    if (currentAnalysis.type === 'sector_distribution') {
      recommendations.push({
        ...
        description: 'احفر أعمق في القطاع الذي يحتوي أكبر عدد من الحوادث',
        confidence: 0.85,
        priority: 'high',
        createdAt: new Date(),
      });
    ...
    if (currentAnalysis.type === 'monthly_trend') {
      recommendations.push({
        ...
        description: 'استخدم البيانات التاريخية للتنبؤ بعدد الحوادث في الأشهر القادمة',
        confidence: 0.7,
        priority: 'medium',
        createdAt: new Date(),
      });
  ...
  generateSmartQuestions(context: UserContext): string[] {
    const questions: string[] = [
      ...
      '📊 كيف تطورت الحوادث خلال الأشهر الستة الماضية؟',
      '🏪 من هم البائعون الأكثر نشاطاً في السوق السوداء؟',
      '🔐 ما هي أنواع البيانات الشخصية الأكثر تسريباً؟',
      '⚠️ ما هي الحوادث ذات الخطورة الحرجة المفتوحة حالياً؟',
  ...
  discoverHiddenRelations(entities: any[]): Recommendation[] {
    ...
    if (frequentSellers.length > 0) {
      ...
      recommendations.push({
        ...
        description: `البائع "${seller}" ظهر في ${count} حوادث. يُنصح بإنشاء قاعدة مراقبة خاصة به`,
        confidence: 0.9,
        priority: 'high',
  ...
  detectPatterns(incidents: any[]): Pattern[] {
    ...
    if (previousCount > 0 && recentCount > previousCount * 1.5) {
      patterns.push({
        id: 'anomaly-spike-1',
        type: 'anomaly',
        description: `زيادة غير عادية في الحوادث: ${recentCount} حادثة هذا الأسبوع مقارنة بـ ${previousCount} الأسبوع الماضي (زيادة ${Math.round((recentCount / previousCount - 1) * 100)}%)`,
        confidence: 0.85,
    ...
    if (topSector && topSector[1] >= 5) {
      patterns.push({
        id: 'trend-sector-1',
        type: 'trend',
        description: `القطاع ${topSector[0]} يشهد نشاطاً متزايداً مع ${topSector[1]} حوادث خلال الأسبوع الماضي`,
        confidence: 0.8,
