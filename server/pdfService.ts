    ...
    try {
      ...
      const llmRes = await invokeLLM({
        messages: [
          {
            role: "system",
            content: `أنت محلل أمن معلومات متخصص في نظام حماية البيانات الشخصية السعودي (PDPL). قدم 5-7 توصيات عملية ومحددة للتعامل مع هذا حالة الرصد. أجب بصيغة JSON فقط: {"recommendations": ["..."], "summary": "..."}`
          },
          {
            role: "user",
            content: `حالة رصد: ${leak.titleAr}\nمستوى التأثير: ${leak.severity}\nالمصدر: ${leak.source}\nالسجلات: ${leak.recordCount}\nالقطاع: ${leak.sector}\nالوصف: ${leak.descriptionAr || leak.description}`
          },
        ],
      });
      try {
  ...
  <div class="page">
    ...
    <div style="background:linear-gradient(180deg,#0a2540 0%,#0c3054 100%);padding:28px 36px;position:relative;z-index:1;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;position:relative;z-index:1;">
        <div style="text-align:right;flex:1;">
          ...
          <p style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:4px;padding-right:2px;">رصد وتوثيق حالات رصد البيانات الشخصية</p>
        </div>
    ...
    <div style="padding:26px 36px;background:linear-gradient(135deg,#f0fdf4,#ecfdf5,#f0f9ff);border-bottom:2px solid #e2e8f0;position:relative;z-index:1;">
      <div style="text-align:center;">
        <h2 style="font-size:21px;font-weight:900;color:#0a2540;margin:0;">توثيق حالة رصد</h2>
    ...
    <div style="padding:28px 36px;position:relative;z-index:1;">
      ...
      <div style="margin-bottom:28px;">
        ...
        <table class="info-table">
          ...
          ${leak.breachMethodAr ? `<tr><td class="label-cell">طريقة حالة الرصد</td><td class="value-cell">${leak.breachMethodAr}</td></tr>` : ""}
      ...
      ${
        ...
        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px 14px;margin-bottom:14px;">
          <p style="font-size:10px;color:#dc2626;margin:0;font-weight:700;">تنبيه: البيانات أدناه عينات توضيحية من حالة الرصد لأغراض التوثيق الرسمي فقط — يُمنع نسخها أو مشاركتها</p>
        </div>
