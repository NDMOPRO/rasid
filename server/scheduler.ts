...
const telegramThreats: ThreatTemplate[] = [
  { title: "Saudi Banking Credentials Dump", titleAr: "حالة رصد بيانات اعتماد مصرفية سعودية", sector: "Banking", sectorAr: "القطاع المصرفي", severity: "critical" as const, piiTypes: ["National ID", "IBAN", "Phone"], records: 15000 },
...
const pasteThreats: ThreatTemplate[] = [
  { title: "Leaked Employee Credentials", titleAr: "حالة رصد بيانات اعتماد الموظفين", sector: "Technology", sectorAr: "قطاع التقنية", severity: "medium" as const, piiTypes: ["Email", "Phone"], records: 1500 },
...
async function runMonitoringScan(platform: "telegram" | "darkweb" | "paste"): Promise<{
  ...
  // Disabled: Real incidents are managed manually, not auto-generated
  // Previously 30% chance of detecting a new leak per scan
  const detected = false; // Math.random() < 0.3;
  ...
  const leakId = `LK-${Date.now().toString(36).toUpperCase()}-${nanoid(6)}`;
  ...
  try {
    ...
    const notifId = await createNotification({
      ...
      titleAr: `حالة رصد جديد: ${threat.titleAr}`,
      message: `${threat.severity.toUpperCase()} severity leak detected on ${platform} with ${threat.records}+ records`,
      messageAr: `تم رصد حالة رصد بتصنيف ${threat.severity === "critical" ? "واسع النطاق" : threat.severity === "high" ? "مرتفع" : threat.severity === "medium" ? "متوسط" : "محدود"} على ${platform === "telegram" ? "تليجرام" : platform === "darkweb" ? "الدارك ويب" : "مواقع اللصق"}`,
      severity: threat.severity,
      relatedId: leakId,
    });
    ...
    broadcastNotification({
      ...
      messageAr: `تم اكتشاف حالة رصد جديد على ${platform === "telegram" ? "تليجرام" : platform === "darkweb" ? "الدارك ويب" : "مواقع اللصق"}`,
      severity: threat.severity,
      relatedId: leakId,
  ...
  try {
    ...
    if (totalLeaksFound > 0) {
      await createNotification({
        ...
        titleAr: `اكتمال مهمة الرصد: تم العثور على ${totalLeaksFound} حالة رصد(ات)`,
