...
const categoryConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  data_leak: { label: "تسرب بيانات", icon: Database, color: "text-red-400" },
...
export default function ThreatRules() {
  ...
  const statItems = [
    { key: "total_rules", label: "إجمالي القواعد", value: stats.total, icon: Crosshair, color: "text-primary", description: "إجمالي القواعد يمثل العدد الكلي لقواعد الصيد والكشف عن التهديدات المتاحة في النظام. هذه القواعد مصممة لتحديد الأنشطة المشبوهة وتسرب البيانات والتهديدات الأمنية الأخرى." },
      ...
      >
        ...
        <div className="relative h-full flex flex-col justify-center px-6 lg:px-10">
          ...
          <p className="text-sm text-muted-foreground max-w-lg">
            25 قاعدة YARA مخصصة للسياق السعودي — كشف تلقائي لحالات الرصد وبيانات الاعتماد وإعلانات البيع
          </p>
        </div>
      </motion.div>
