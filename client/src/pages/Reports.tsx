...
const policyGaps = [
  ...
  },
  {
    ...
    sector: "اتصالات",
    urgency: "high",
    description: "قطاع الاتصالات الأكثر تعرضاً — الحاجة لمعايير تشفير إلزامية",
  ...
  {
    id: "PG-003",
    title: "إطار توثيق حالات الرصد",
    titleEn: "Incident Reporting Framework",
    sector: "عام",
    urgency: "high",
    description: "لا يوجد إطار موحد لتوثيق ورصد حالات تسرب البيانات الشخصية",
    recommendation: "إنشاء إطار توثيق موحد مع جداول زمنية محددة",
    progress: 60,
    details: "حالياً لا يوجد إطار موحد لتوثيق حالات رصد تسرب البيانات خلال فترة زمنية محددة. هذا يؤدي لتأخر الرصد وزيادة الأضرار. الإطار المقترح يشمل: توثيق خلال 72 ساعة، تصنيف الحالات، وخطة متابعة.",
...
export default function Reports() {
  ...
  const handleExportReport = async () => {
    try {
      ...
      const lines = [
        ...
        "",
        "=== إحصائيات عامة ===",
        `إجمالي حالات الرصد: ${data?.stats?.totalLeaks ?? 0}`,
        `حالات رصد جديدة: ${data?.stats?.newLeaks ?? 0}`,
        `إجمالي العدد المُدّعى للسجلات: ${data?.stats?.totalRecords ?? 0}`,
        `مصادر الرصد النشطة: ${data?.stats?.activeMonitors ?? 0}`,
        `بيانات شخصية مكتشفة: ${data?.stats?.piiDetected ?? 0}`,
        "",
        "=== ملخص حالات الرصد ===",
  ...
  return (
    <div className="space-y-6">
      ...
      <DetailModal open={activeModal === "barChart"} onClose={() => setActiveModal(null)} title="تفاصيل العدد المُدّعى للسجلات شهرياً" icon={<BarChart3 className="w-5 h-5 text-cyan-400" />} maxWidth="max-w-4xl">
        <div className="space-y-4">
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyTrends}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-right py-2 px-3 text-xs font-medium text-muted-foreground">العدد المُدّعى</th>
