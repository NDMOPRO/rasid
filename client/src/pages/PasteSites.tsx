...
export default function PasteSites() {
  ...
  const stats = [
    { id: "monitored-sites", label: "مواقع مراقبة", value: pasteChannels.length, color: "text-amber-400", icon: Server, description: "إجمالي عدد مواقع اللصق التي تتم مراقبتها حاليًا بحثاً عن حالات رصد محتملة." },
    ...
    { id: "flagged", label: "موثّقة", value: pasteEntries.filter((p) => p.status === "flagged").length, color: "text-red-400", icon: ShieldAlert, description: "عدد اللصقات التي تم تحديدها على أنها تحتوي على بيانات شخصية مكتشفة وتم توثيقها." },
  ];
