    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { key: "totalLeaks", label: "تسريبات مرتبطة", value: stats?.totalLeaks ?? 0, icon: TrendingUp, color: "text-amber-400", borderColor: "border-amber-500/20", bgColor: "bg-amber-500/5" },
        ].map((stat, i) => (
        ...
        <div className="space-y-3">
          {sellers?.filter(s => s.riskLevel === "critical").map(seller => (
            ...
            >
              ...
              <p className="text-[10px] text-muted-foreground">{seller?.totalLeaks ?? 0} تسريب • {(seller?.totalRecords ?? 0).toLocaleString()} سجل</p>
            </div>
          ))}
        </div>
      </DetailModal>
